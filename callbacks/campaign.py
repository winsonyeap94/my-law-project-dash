...
def cluster_marketing_allocation(
    n_click, budget, roi, cluster_data, product_data, product_cost_data, product_profit_data,
    customer_data_costprofit
):
    ...
    cluster_product_assignment_data = pd.DataFrame(api_response['cluster_product_assignment_data']).reset_index(
        drop=True)
...
def customer_marketing_allocation(
    n_clicks, budget, roi, cluster_data, product_data, product_cost_data, product_profit_data,
    customer_data_costprofit
):
    ...
    revised_budget = 'RM {:,.2f}'.format(
        pd.DataFrame(api_response['tactical_expected_money'])['money_expected_cost'][0].astype(float))
    ...
    cluster_product_cust_assignment_data = pd.DataFrame(
        api_response['cluster_product_customer_assignment_data']).reset_index(drop=True)
